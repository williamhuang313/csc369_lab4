...
public class HadoopApp {
	...
	if (otherArgs.length < 3) {
	    ...
	    FileInputFormat.addInputPath(job, new Path(otherArgs[1]));
	    FileOutputFormat.setOutputPath(job, new Path(otherArgs[2]));
	} else if ("R1_AL_Processing".equalsIgnoreCase(otherArgs[0])) {
	    job.setReducerClass(R1_AL_Processing.ReducerImpl.class);
	    job.setMapperClass(R1_AL_Processing.MapperImpl.class);
	    job.setOutputKeyClass(R1_AL_Processing.OUTPUT_KEY_CLASS);
	    ...
	    FileInputFormat.addInputPath(job, new Path(otherArgs[1]));
	    FileOutputFormat.setOutputPath(job, new Path(otherArgs[2]));
	} else if ("R1_Sort".equalsIgnoreCase(otherArgs[0])) {
	    job.setReducerClass(R1_Sort.ReducerImpl.class);
	    job.setMapperClass(R1_Sort.MapperImpl.class);
	    job.setOutputKeyClass(R1_Sort.OUTPUT_KEY_CLASS);
	    job.setOutputValueClass(R1_Sort.OUTPUT_VALUE_CLASS);
	    FileInputFormat.addInputPath(job, new Path(otherArgs[1]));
	    FileOutputFormat.setOutputPath(job, new Path(otherArgs[2]));} 
	else if ("R2_Join".equalsIgnoreCase(otherArgs[0])) {

	    MultipleInputs.addInputPath(job, new Path(otherArgs[1]),
					TextInputFormat.class, R2_Join.ALMapper.class );
	    MultipleInputs.addInputPath(job, new Path(otherArgs[2]),
					TextInputFormat.class, R2_Join.CountryMapper.class ); 

	    job.setReducerClass(R2_Join.JoinReducer.class);

	    job.setOutputKeyClass(R2_Join.OUTPUT_KEY_CLASS);
	    job.setOutputValueClass(R2_Join.OUTPUT_VALUE_CLASS);
	    FileOutputFormat.setOutputPath(job, new Path(otherArgs[3]));

	} else if ("R2_Reduce".equalsIgnoreCase(otherArgs[0])) {
	    job.setReducerClass(R2_Reduce.ReducerImpl.class);
	    job.setMapperClass(R2_Reduce.MapperImpl.class);
	    job.setOutputKeyClass(R2_Reduce.OUTPUT_KEY_CLASS);
	    job.setOutputValueClass(R2_Reduce.OUTPUT_VALUE_CLASS);
	    FileInputFormat.addInputPath(job, new Path(otherArgs[1]));
	    FileOutputFormat.setOutputPath(job, new Path(otherArgs[2]));
	} else {
